...
from visualiser.pages.embedding_visualisation_layout import (
...
class EmbeddingVisualiser:
    ...
    def filter_data(self, dataset_names, option):
        filtered_dfs = []
        for dataset_name in dataset_names:
            dataset_df = self.dataframes_dict[dataset_name].copy()
            dataset_df["dataset"] = dataset_name  # Add a column to distinguish datasets
            if option == "all":
                filtered_dfs.append(dataset_df)
            elif option == "true_positive":
                filtered_dfs.append(
                    dataset_df[dataset_df["label"] == dataset_df["pred"]]
                )
    ...
    def update_graph(
        ...
        dataset_df = self.filter_data(dataset_names, selected_option)

        # Converting string values into appropriate formats
        n_neighbors = int(n_neighbors)
        min_dist = float(min_dist)
        marker_size = int(marker_size)

        # Convert labels to string and concatenate dataset name if multiple datasets
        dataset_df["label"] = dataset_df["label"].astype(str)
        # if len(dataset_names) > 1:
        #     dataset_df["label"] = dataset_df["dataset"] + ": " + dataset_df["label"]
        ...
        if dimensions == "2D":
            ...
            fig.update_scenes(dragmode=True, aspectmode="cube")

        # Make plot responsive and maintain aspect ratio
        fig.update_layout(
            autosize=True,
            width=None,
            height=900,
    ...
    def display_hover_image(self, hoverData, dataset_name):
        ...
        if type(dataset_name) == type([]):
            dataset_name = dataset_name[0]
        # print(dataset_name)
        # print(type(self.dataframes_dict))
        # print(self.dataframes_dict)
