...
from visualiser.pages import (
    data_insights_layout,
    # model_performance_layout,
    embedding_visualisation_layout,
    # dataset_suggester_layout,
)
import visualiser.pages.data_insights as data_insights

# import visualiser.pages.model_performance as model_performance
from visualiser.pages.embedding_visualisation import EmbeddingVisualiser

# import visualiser.pages.dataset_suggester as dataset_suggester
...
class App:
    ...
    def run(self, host=None, debug=True, use_reloader=False, port=8051):
        if host is None:
            self.app.run_server(debug=debug, use_reloader=use_reloader, port=port)
        else:
            self.app.run_server(
                host=host, debug=debug, use_reloader=use_reloader, port=port
            )
...
if __name__ == "__main__":
    ...
    dataframes_dict = {
        "sample_dataset": sample_dataframe,
        "another_dataset": another_dataframe,
    }
    root_paths = {
        "sample_dataset": "/path/to/sample_dataset",
        "another_dataset": "/path/to/another_dataset",
    }
